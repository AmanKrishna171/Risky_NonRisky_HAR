...
while(cap.isOpened()):
    ...
    if ret == True:
        ...
        if len(clips) == args.clip_len:
            ...
            # cv2.putText(image, label, (35, 55),
            #             cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 3, 
            #             lineType=cv2.LINE_AA)

            # Calculate the width and height of the text box
            (text_width, text_height) = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 3, 4)[0]

            # Set the text start position
            text_x, text_y = 20, 75

            # Increase the padding to make the rectangle bigger
            padding = 30

            # Draw a filled rectangle as the background of the text
            cv2.rectangle(image, (text_x - padding, text_y + padding), (text_x + text_width + padding, text_y - text_height - padding), (0, 0, 0), -1)

            # Put the text on the image
            cv2.putText(image, label, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 3, (255, 255, 255), 4, lineType=cv2.LINE_AA)
